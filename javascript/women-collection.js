document.addEventListener("DOMContentLoaded", function () {
  ...
  // Get the Women Collection elements
  const womenCollection = document.getElementById("Woman");
  if (!womenCollection) return; // Exit if Women collection doesn't exist
  
  const womenProductsGrid = womenCollection.querySelector('.products-grid[data-collection="women"]');
  const showMoreBtn = womenCollection.querySelector(".show-more-btn");
  const sortSelect = womenCollection.querySelector("#sort-select");
  const resultsCountSpan = womenCollection.querySelector(".results-count span");
  
  // Exit if elements don't exist
  if (!womenProductsGrid || !showMoreBtn) return;
  
  // Get all products and store original order
  const allProducts = Array.from(womenProductsGrid.querySelectorAll(".product-item"));
  ...
  function sortProducts() {
    ...
    filteredProducts.forEach((product) => {
      womenProductsGrid.appendChild(product);
    });
  ...
  function addFilterButtons() {
    ...
    let filterContainer = womenCollection.querySelector(".collection-filters");
    if (!filterContainer) {
      filterContainer = document.createElement("div");
      filterContainer.className = "collection-filters";
      const collectionHeader = womenCollection.querySelector(".collection-header");
      if (collectionHeader) {
  ...
  const wishlistButtons = womenCollection.querySelectorAll(".wishlist-btn");
  ...
  const addToCartButtons = womenCollection.querySelectorAll(".add-to-cart");
